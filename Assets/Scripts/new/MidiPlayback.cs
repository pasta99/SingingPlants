...
public class MidiPlayback : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        var midiFile = LoadMidi(midiFileName);
        var t = midiFile.GetTempoMap();
        InitializeFilePlayback(midiFile);
        StartPlayback();
    }

    private MidiFile LoadMidi(string fileName) {
        MidiFile midiFile = MidiFile.Read($"Assets/{fileName}");
        Debug.Log("Loaded Midi file successfully!");
        return midiFile;
    }
    ...
    private void InitializeOutputDevice()
    {
        ...
        _outputDevice = transform.GetComponent<MidiHandler>();
